...
public class Transaction
  ...
  private static void rollbackNewAssets(ExecutionContext context, User user, OperationResult result,
    ...
    String rollbackLabel = ProjectMessageResource.getMessageText("rollback.label");

    // Bail if there are no assets to process.
    if (assetsToProcess == null || assetsToProcess.size() == 0) {
      result.addInfoMessage(rollbackLabel, ProjectMessageResource.getMessageText(
          "rollback.info.no.new.assets"));
      return;
    }

    result.addInfoMessage(rollbackLabel, ProjectMessageResource.getMessageText(
        "rollback.info.processing.new.assets", assetsToProcess.size()));
    ...
    for (Map.Entry<String, String> entry : assetsToProcess.entrySet()) {
      ...
      try {
        result.addInfoMessage(rollbackLabel, ProjectMessageResource.getMessageText(
            "rollback.info.processing.new.asset", label, id));
        ...
        result.addWarning(rollbackLabel, new RSuiteException(RSuiteException.ERROR_INTERNAL_ERROR,
            ProjectMessageResource.getMessageText("rollback.warn.unable.to.process.new.asset",
                label, id, e.getMessage()), e));
        continue;
      }
    }
  }
  ...
  private static void rollbackUpdatedAssets(ExecutionContext context, User user,
    ...
    if (assetsToProcess == null || assetsToProcess.size() == 0) {
      result.addInfoMessage(rollbackLabel, ProjectMessageResource.getMessageText(
          "rollback.info.no.updated.assets"));
      return;
    }

    result.addInfoMessage(rollbackLabel, ProjectMessageResource.getMessageText(
        "rollback.info.processing.updated.assets", assetsToProcess.size()));
    ...
    for (Map.Entry<String, String> entry : assetsToProcess.entrySet()) {
      ...
      try {
        result.addInfoMessage(rollbackLabel, ProjectMessageResource.getMessageText(
            "rollback.info.processing.updated.asset", label, id));
        ...
        if (versionSpecifier == null) {
          result.addWarning(rollbackLabel, new RSuiteException(RSuiteException.ERROR_INTERNAL_ERROR,
              ProjectMessageResource.getMessageText("rollback.warn.updated.asset.has.one.version",
                  label, id)));
          continue;
        }
        ...
        result.addWarning(rollbackLabel, new RSuiteException(RSuiteException.ERROR_INTERNAL_ERROR,
            ProjectMessageResource.getMessageText("rollback.warn.unable.to.process.updated.asset",
                label, id, e.getMessage()), e));
        continue;
      }
    }
