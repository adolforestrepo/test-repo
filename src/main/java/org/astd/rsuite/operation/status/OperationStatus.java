package org.astd.rsuite.operation.status;
...
public enum OperationStatus {
  ...
  QUEUED(true, true, "queued", "Queued"),
  IN_PROGRESS(true, true, "in-progress", "In-Progress"),
  SUCCESSFUL(true, false, "successful", "Successful"),
  FAILED(true, false, "failed", "Failed"),
  ABORTED(true, false, "aborted", "Aborted"),
  SKIPPED(false, false, "skipped", "Skipped"),
  EXCLUDED(true, false, "excluded", "Excluded");
  ...
  private OperationStatus(
      boolean allowSet, boolean outstanding, String counterName, String displayName) {
  ...
  public static OperationStatus get(String name, boolean throwIfInvalid) throws RSuiteException {
    ...
    if (throwIfInvalid) {
      throw new RSuiteException(RSuiteException.ERROR_PARAM_INVALID, ProjectMessageResource
          .getMessageText("operation.status.error.unknown.status", name));
    }
    return null;
  }
