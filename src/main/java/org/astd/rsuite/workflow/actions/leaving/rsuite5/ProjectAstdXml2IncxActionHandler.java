	...
	public void execute(WorkflowContext context) throws Exception {
		...
		if (xmlMoId == null || "".equals(xmlMoId)) {
			...
			if (moList == null || moList.isEmpty()) {
				mo = context.getManagedObjectService().getObjectByAlias(context.getAuthorizationService().getSystemUser(), fileName + ".xml");	
				if (mo == null) {
					reportAndThrowRSuiteException("Failed to get an MO using the fileName parameter with the value \"" + fileName + "\"");
				}
			} else {
				MoWorkflowObject moObject = moList.getMoList().get(0); // First item in list should be course script MO.
				mo = context.getManagedObjectService().getManagedObject(context.getAuthorizationService().getSystemUser(), moObject.getMoid());
				if (mo == null) {
			...
			mo = context.getManagedObjectService().getManagedObject(context.getAuthorizationService().getSystemUser(), xmlMoId);
			if (mo == null) {
		...
		try {
			...
			reportAndThrowRSuiteException("Exception generating InDesign from mo: " + e.getLocalizedMessage());
		} finally {
			...
			if (exceptionOccured) {
				...
				.append(context.getVariable(ProjectAstdWorkflowConstants.EXCEPTION_MESSAGE_VAR));
			} else {
