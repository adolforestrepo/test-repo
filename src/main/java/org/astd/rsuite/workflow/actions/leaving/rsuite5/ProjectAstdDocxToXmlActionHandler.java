...
public class ProjectAstdDocxToXmlActionHandler extends BaseWorkflowAction
	...
	public void execute(WorkflowContext context) throws Exception {
		...
		if (xsltGraphicRenameUri == null
				|| resolveExpression(xsltGraphicRenameUri) == null
				|| resolveVariablesAndExpressions(xsltGraphicRenameUri
						.getExpressionText()) == null) {
			xsltGfxRenameUri = "rsuite:/res/plugin/dita4publishers/xslt/word2dita/rewriteDocxGraphics.xsl";
		} else {
			xsltGfxRenameUri = resolveVariablesAndExpressions(xsltGraphicRenameUri
					.getExpressionText());
			// xsltGfxRenameUri =
			// resolveVariablesAndExpressions(xsltGfxRenameUri);
		}

		String gfxPrefix = "";
		if (graphicsPrefix == null
				|| resolveExpression(graphicsPrefix) == null
				|| resolveVariablesAndExpressions(graphicsPrefix
						.getExpressionText()) == null) {
			gfxPrefix = "";
		} else {
			gfxPrefix = resolveVariablesAndExpressions(graphicsPrefix
					.getExpressionText());
		}
		...
		wfLog.info("Effective output directory is \""
		...
		if (docxMoId == null || resolveExpression(docxMoId) == null) {
			...
			MoWorkflowObject moObject = (MoWorkflowObject) moList.getMoList()
					.get(0);
			...
			if ((mo instanceof ContentAssembly)) {
				reportAndThrowRSuiteException("Specified MO [" + docxMoId
						+ "] is a content assembly or content assembly node.");
			}
			if (!Docx2Xml.isTransformationApplicable(mo)) {
		...
		String fileNamePrefix = resolveVariablesAndExpressions(getParameterWithDefault(
				"fileNamePref", ""));
		String submapNamePrefix = resolveVariablesAndExpressions(getParameterWithDefault(
				"submapNamePref", "map"));
		...
		bean = new Docx2XmlBean(context, xsltUriVar, styleMapUriVar,
				mapFileName, topicFileName);
		...
		String reportFileName = fileNameBase + "_docx2dita_at_"
				+ DitaOtOptions.getNowString() + ".txt";
		String xformReportIdVarName = getParameterWithDefault(
				"xformReportIdVarName", "xformReportId");
		...
		String xformReportFileNameVarName = getParameterWithDefault(
				"xformReportFileNameVarName", "xformReportFileName");
		...
		params.put(
				"debug",
				resolveVariablesAndExpressions(getParameterWithDefault("debug",
						"false")));
		...
		try {
			...
			StringBuilder reportStr = new StringBuilder(
					"DOCX 2 DITA Transformation report\n\n")
					.append("Source DOCX: ").append(mo.getDisplayName())
					.append(" [").append(mo.getId()).append("]\n")
					.append("Result file: ")
					.append(resultMapFile.getAbsolutePath()).append("\n")
					.append("Time performed: ")
					.append(DitaOtOptions.getNowString()).append("\n\n")
					.append(logger.getLogString());

			if (exceptionOccured) {
				reportStr.append(" + [ERROR] Exception occurred: ").append(
		...
		wfLog.info("XML generated in location \""
				+ resultMapFile.getAbsolutePath() + "\"");
