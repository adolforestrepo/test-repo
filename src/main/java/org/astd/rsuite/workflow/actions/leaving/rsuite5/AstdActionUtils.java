...
public class AstdActionUtils implements TempWorkflowConstants {
	 ...
	 * Create a new CA.
	 * <p>
	 * This method will create any folder path components if
	 ...
	 *             If an error occurs.
	 */
	public static String createCA(RepositoryService repositoryService,
			User user, String ca, String parentUri, WorkflowContext context)
			throws RSuiteException {
		...
		createFolder(user, parentUri, context);
		workflowLog
		...
		workflowLog.info("AstdActionUtils: CA \"" + ca + "\" created. ID is "
				+ moid);
	 ...
	 *             If an error occurs.
	 */
	public static String createFolder(User user, String folder,
			WorkflowContext executionContext) throws RSuiteException {

		/*
		 * Log workflowLog = executionContext.getWorkflowLog();
		 * 
		 * if (folder == null || "".equals(folder) || "/".equals(folder)) return
		 * null; if (!folder.startsWith("/")) folder = "/" + folder; if
		 * (folder.endsWith("/")) folder = folder.substring(0, folder.length() -
		 * 1);
		 * 
		 * workflowLog.info(
		 ...
		 * 
		 * 
		 * String folderpath = null; String p = "/"; String f = folder;
		 * 
		 * if (!service.hasFolder(folder + "/")) { int index =
		 * folder.lastIndexOf("/");
		 * 
		 * if (index > 0) { p = folder.substring(0, index); f =
		 * folder.substring(index + 1); } else if (index == 0) { f =
		 * folder.substring(1); } workflowLog
		 * .info("AstdActionUtils: Before createFolder, p=\"" + p + "\", f=\"" +
		 * f + "\""); folderpath = service.createFolder(user, p, f, true, null,
		 * true) .getPath(); workflowLog.info("AstdActionUtils: Folder \"" +
		 * folderpath + "\" created.");
		 * 
		 * } else { folderpath = service.getFolder(user, folder +
		 * "/").getPath(); workflowLog.info("AstdActionUtils: Folder \"" +
		 * folder + "\" already exists, not created."); }
		 */
	 ...
	 *             If an error occurs.
	 */
	public static boolean isArticleContentsLocked(ContentAssemblyItem ca)
			throws RSuiteException {
		List<? extends ContentAssemblyItem> items = ca.getChildrenObjects();
		...
		for (ContentAssemblyItem item : items) {
			...
			if (isArticleContentsLocked(item))
				return true;
		}
		return false;
	}
	 ...
	 * @param mor
	 *            MO reference.
	 * @return <tt>true</tt> if reference to a non-XML MO, otherwise
	 *         <tt>false</tt>.
	 */
	public static boolean isNonXml(ManagedObjectReference mor) {
		// XXX: Current implementation will throw not-implemented exception
		// when trying to retrieve the namespace URI.
		/*
		 * String ns = mor.getNamespaceURI(); if (ns == null ||
		 * !ns.equals(AstdWorkflowConstants.RSUITE_NS_URI)) { return false; }
		 */
		String lname = mor.getLocalName();
		if (lname != null && lname.equals(RSUITE_NON_XML_LOCAL_NAME)) {
			return true;
		}
		return false;
	}
	 ...
	 * <p>
	 * This method is used for supporting reassignment of articles.
	 * </p>
	 * 
	 * @param user
	 *            User making the request.
	 ...
	 *             If an error occurs.
	 */
	public static String renameArticleNonXmlMo(User user,
			ManagedObjectService mosvc, Log log, String moid, String basename)
			throws RSuiteException {
		...
		String cType = (ext.charAt(0) == '.') ? ext.substring(1).toUpperCase()
				: ext.toUpperCase();
		String newName = basename + ext;
		log.info("New name to set: " + newName);
		log.info("File content type: " + cType);

		ObjectUpdateOptions options = ObjectUpdateOptions
				.constructOptionsForNonXml(newName, cType);
		...
		try {
			...
			mosvc.checkIn(user, moid, VersionType.MINOR, "Article reassign",
					false);
		}
		return newName;
	}
	 ...
	 * <p>
	 * If <var>off</var> and <var>len</var> parameters would cause filling to
	 * exceed <var>dest</var>, a new byte array is allocated containing a copy
	 * of <var>dest</var> contents and enough room to satisfy the fill length.
	 * </p>
	 * <p>
	 * This method will continue reading until <var>len</var> bytes have been
	 * read.
	 * </p>
	 * 
	 * @param in
	 *            {@link InputStream} to get data from.
	 * @param dest
	 *            Byte array to fill.
	 * @param off
	 *            Starting fill offset into <var>dest</var>.
	 * @param len
	 *            Number of bytes to fill.
	 * @return Reference to filled byte array, which is <var>dest</var> unless a
	 *         new array was allocated because fill length exceeded
	 *         <var>dest</var> bounds.
	 * @throws IOException
	 *             if an I/O error occurs while reading from <var>in</var>.
	 */
	public static byte[] fillBuffer(InputStream in, byte[] dest, int off,
		...
		for (n = 0; (len > 0) && (n = in.read(dest, off, len)) >= 0; off += n, len -= n)
			;
		return dest;
	}
	 ...
	 *            MO of article CA.
	 * @param user
	 *            User to execute action under.
	 * @return <tt>true</tt> if field was cleared, otherwise <tt>false</tt> if
	 *         field did not exist.
	 * @throws RSuiteException
	 */
	public static boolean clearArticlePidLmdField(ExecutionContext context,
			ManagedObject mo, User user) throws RSuiteException {
		// Metadata value must be out of sync (e.g., process was manually
		// killed)
		...
		for (MetaDataItem candItem : items) {
			...
			if (candItem.getName().equals(ASTD_ARTICLE_PID_LMD_FIELD)) {
				pIdItem = candItem;
				break;
			}
		}
		if (pIdItem != null) { // Should never be null if we got here.
			context.getManagedObjectService().removeMetaDataEntry(user,
					mo.getId(), pIdItem);
			return true;
		}
		return false;
	 ...
	 * @throws RSuiteException
	 */
	public static String getCAnodeIdbyName(ContentAssembly CA,
			String DisplayName) throws RSuiteException {

		String canode_Id = "";

		List<? extends ContentAssemblyItem> CAs = CA.getChildrenObjects();

		for (ContentAssemblyItem caitme : CAs) {
			if (caitme.getDisplayName().equalsIgnoreCase(DisplayName)
					&& caitme.getObjectType().equals(
							ObjectType.CONTENT_ASSEMBLY_NODE)) {
				return caitme.getId();
			}
		}
	 ...
	 * @throws RSuiteException
	 */
	public static String convertCAnodeToCA(ExecutionContext context,
			String CAnodeId) throws RSuiteException {
		...
		ManagedObject CAnode = moSrv.getManagedObject(user, CAnodeId);
		String parentID = moSrv.getParentManagedObjectId(user, CAnodeId);
		List<? extends ContentAssemblyItem> objects = caSrv
				.getContentAssemblyNodeContainer(user, CAnodeId)
				.getChildrenObjects();

		ContentAssembly articleCA = caSrv.createContentAssembly(user, parentID,
				CAnode.getDisplayName(), caCreateOp);

		for (ContentAssemblyItem mo : objects) {
			caSrv.attach(user, articleCA.getId(), moSrv.getManagedObject(user, mo.getId()).getTargetId(),
					new ObjectAttachOptions()); 
		}

		caSrv.deleteCANode(user, CAnodeId);

		return articleCA.getId();
	 ...
	 * 
	 * @param mo
	 *            MO to get content from.
	 * @param encoding
	 *            Text encoding to use; if null, UTF-8 is used.
	 * @return String contain contents.
	 * @throws RSuiteException
	 *             If an I/O error occurs.
	 */
	public static String getMoText(ManagedObject mo, String encoding)
			throws RSuiteException {
	 ...
	 * <p>
	 * At this time, this function assumes the classifications is stored in the
	 * &lt;classification&gt; child element of the &lt;prolog&gt; descendent of
	 * the root element of the MO.
	 * </p>
	 * <p>
	 * TODO: Modify code to key off of DITA type values instead of element
	 * names.
	 * </p>
	 * <p>
	 * The classification XML content will be wrapped in a &lt;RESULT&gt;
	 * element.
	 * </p>
	 * 
	 * @param context
	 *            Execution context.
	 * @param log
	 *            Log to use to print any log messages.
	 * @param moid
	 *            ID of MO to get classification data of.
	 * @return XML string containing classification data.
	 * @throws RSuiteException
	 *             If an error occurs retrieving data.
	 */
	public static String getClassificationXmlOfMo(ExecutionContext context,
			Log log, String moid) throws RSuiteException {
		RepositoryService rs = context.getRepositoryService();
		String moPath = rs.getXPathToObject(moid);
		StringBuilder buf = new StringBuilder(256);
		buf.append("<RESULT>").append("{").append(moPath).append("/prolog/")
				.append(CLASSIFICATION_GI).append("}").append("</RESULT>");
		log.info("Executing query: " + buf);
		String results = context.getRepositoryService().queryAsString(
				buf.toString());
		log.info("Query results: " + results);
		return results;
	}

	private static String[] postPrologCandidates = { "<subsection", "<sidebar",
			"<interview-question", "<glossentry", "<glossgroup" };
	 ...
	 *            Execution context.
	 * @param user
	 *            User to perform operations under.
	 * @param log
	 *            Log to use for logging messages.
	 * @param moid
	 *            ID of MO to set.
	 * @param xml
	 *            &lt;classification&gt element and its content.
	 * @throws RSuiteException
	 *             if an error occurs.
	 */
	public static void setClassificationXmlForMo(ExecutionContext context,
			User user, Log log, String moid, String xml) throws RSuiteException {
	 ...
	 */
	public static void setClassificationXmlForMo(ExecutionContext context,
			User user, Log log, ManagedObject mo, String xml)
			throws RSuiteException {
		ManagedObjectService mos = context.getManagedObjectService();
		...
		StringBuilder newXml = new StringBuilder(moXml.length() + xml.length()
				+ 20);
		...
		if (start < 0) {
			...
			if (end < 0) {
				...
				if (end < 0) {
					// Should not get here(?)
					throw new RSuiteException("MO (" + moid
							+ ") has malformed <prolog>");
				}
		...
		try {
			...
			if (doCheckIn) {
				...
				try {
					...
					log.error("Unable to rollback MO do to earlier error: "
							+ rse2, rse2);
				}
			}
		...
		if (doCheckIn) {
			log.info("Checking in MO (" + moid + ") under user "
					+ user.getUserId());
			mos.checkIn(user, moid, VersionType.MINOR, "Classification change",
					true);
		}
	}
}
