...
/**
 * Converts an ASTD XML MO to icml using the specified transform and style
 * catalog.
 */
public class ProjectAstdXml2IcmlActionHandler extends BaseWorkflowAction
	...
	/**
	 * Optional. Specifies the base name of the icml file to generate. If the
	...
	/**
	 * URL of the XSLT that does the transforming from XML to icml
	 */
	public static final String XSLT_URI_PARAM = "xsltUriFromWorkflow";
	/**
	 * URL of the style catalog to use for the result icml file.
	 */
	 ...
	 * executive summary icml file.
	 * <p>
	 * If not specified and an executive summary file is generated during
	 * conversion, then the variable <tt>icmlExecSummaryPath</tt> is used.
	 * </p>
	 */
	 ...
	 * <p>
	 * If not specified and an executive summary file is generated during
	 * conversion, then the variable <tt>icmlExecSummaryFile</tt> is used.
	 * </p>
	 */
	...
	public void execute(WorkflowContext context) throws Exception {
		...
		context.setVariable(DEFAULT_EXEC_SUMMARY_FILE_VAR_NAME, null);
		...
		String fileNameExp = resolveVariablesAndExpressions(fileName
				.getExpressionText());
		String xmlMoIdExp = resolveVariablesAndExpressions(xmlMoId
				.getExpressionText());
		...
		if (xmlMoIdExp == null || "".equals(xmlMoIdExp)) {
			...
			if (moList == null || moList.isEmpty()) {
				mo = context.getManagedObjectService().getObjectByAlias(
						context.getAuthorizationService().getSystemUser(),
						fileNameExp + ".xml");
				if (mo == null) {
					reportAndThrowRSuiteException("Failed to get an MO using the fileName parameter with the value \""
							+ fileNameExp + "\"");
				}
			} else {
				MoWorkflowObject moObject = moList.getMoList().get(0); // First
																		// item
																		// in
																		// list
																		// should
																		...
																		// script
																		// MO.
				mo = context.getManagedObjectService().getManagedObject(
						context.getAuthorizationService().getSystemUser(),
						moObject.getMoid());
				if (mo == null) {
				...
				if (fileNameExp == null || "".equals(fileNameExp)) {
					...
					if (aliases.length > 0) {
						// fileName =
						// FilenameUtils.getBaseName(aliases[0].toString());
						fileNameExp = FilenameUtils.getBaseName(aliases[0]
								.getText());
					} else {
			...
			mo = context.getManagedObjectService().getManagedObject(
					...
					xmlMoIdExp);
			if (mo == null) {
				reportAndThrowRSuiteException("Failed to find MO with ID ["
						+ xmlMoIdExp + "]");
			}
		}

		File resultInxFile = new File(outputDir, fileNameExp + ".icml");

		String reportFileName = fileNameExp + "_docx2dita_at_" + getNowString()
				+ ".txt";
		String xformReportIdVarName = getParameterWithDefault(
				XFORM_REPORT_ID_VAR_NAME_PARAM, XFORM_REPORT_ID_VARNAME);
		...
		String xformReportFileNameVarName = getParameterWithDefault(
				XFORM_REPORT_FILE_NAME_VAR_NAME_PARAM,
				XFORM_REPORT_FILE_NAME_VARNAME);
		...
		try {
			...
			bean.generateInDesignFromTopic(mo.getElement().getOwnerDocument(),
			...
			if (resultInxExecSummaryFile.exists()) {
				...
				/* We need set the property even if there is not executive summary */
				context.setVariable(DEFAULT_EXEC_SUMMARY_FILE_VAR_NAME, null);
			}
		} catch (Exception e) {
			exceptionOccured = true;
			reportAndThrowRSuiteException("Exception generating InDesign from mo: "
					+ e.getLocalizedMessage());
		} finally {
			StringBuilder reportStr = new StringBuilder(
					"XML to InCopy Transformation report\n\n")
					.append("Source XML: ").append(mo.getDisplayName())
					.append(" [").append(mo.getId()).append("]\n")
					.append("Result file: ")
					.append(resultInxFile.getAbsolutePath()).append("\n")
					.append("Time performed: ").append(getNowString())
					.append("\n\n").append(logger.getLogString()).append("\n");
			if (exceptionOccured) {
				reportStr
						.append(" + [ERROR] Exception occurred: ")
						.append(context
								.getVariable(ProjectAstdWorkflowConstants.EXCEPTION_MESSAGE_VAR));
			} else {
				reportStr.append(" + [INFO] Process finished normally");
			}

			StoredReport report = context.getReportManager().saveReport(
					reportFileName, reportStr.toString());
			copiedReportFile = new File(resultInxFile.getParentFile(),
					report.getSuggestedFileName());
			wfLog.info("Generation report in location  \""
					+ copiedReportFile.getAbsolutePath() + "\"");
