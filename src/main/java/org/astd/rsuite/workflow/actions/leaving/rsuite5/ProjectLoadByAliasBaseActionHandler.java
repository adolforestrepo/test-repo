 ...
 * <p>
 * If the alias already exists, the MO associated with the alias will be
 * updated. If the alias does not exist, a new MO is created and an alias is
 * created for it.
 * </p>
 **/
public abstract class ProjectLoadByAliasBaseActionHandler extends
		BaseWorkflowAction implements TempWorkflowConstants {

	/**
	 * Optional. Parameter to take the name of the alias to use for a managed
	 * object.
	 */
	public static final String ALIAS_PARAM = "alias";

	/**
	 * Optional. MO ID of the CA node to attach the loaded or updated MO to if
   ...
   */
	...
	protected void doLoadOrUpdate(WorkflowContext context, Log wfLog,
			File inFile, ObjectSource src, String parentMoId)
		...
		if (mos.size() == 0) {
			wfLog.info("No existing MO with alias \"" + alias
					+ "\", creating new MO...");
			String[] aliases = { alias };
			ObjectInsertOptions options = new ObjectInsertOptions(alias,
					aliases, null, true);
			options.setDisplayName(inFile.getName());
			newMo = moService.load(sys, src, options);
			wfLog.info("Created new MO \"" + newMo.getDisplayName() + "["
					+ newMo.getId() + "]");
		} else if (mos.size() > 1) {
			throw new RSuiteException(0, "Found " + mos.size()
					+ " with alias \"" + alias
					+ ". There should be at most one such MO.");
		} else {
			// Found an existing MO, update it with the new content.
			ManagedObject mo = mos.get(0);
			wfLog.info("Found existing MO \"" + mo.getDisplayName() + "["
					+ mo.getId() + "], updating with XML from incoming file...");
			moService.checkOut(sys, mo.getId());
			ObjectUpdateOptions options = ObjectUpdateOptions
					.constructOptionsForNonXml(inFile.getName(),
							FilenameUtils.getExtension(inFile.getName()));
			options.setValidate(true);
			try {
				newMo = moService.update(sys, mo.getId(), src, options);
			} catch (Exception e) {
				String msg = "Unexpected failure on update of mo ["
						+ mo.getId() + "]: " + e.getMessage();
				reportAndThrowRSuiteException(msg);
			} finally {
				moService.checkIn(sys, newMo.getId(), VersionType.MINOR,
						commitMessage, true);
			}
			wfLog.info("MO updated, new display name is \""
					+ newMo.getDisplayName() + "\"");
		}

		if (parentMoId != null && !"".equals(parentMoId.trim())) {
			wfLog.info("Parent MO ID specified, trying to find and attach...");
			try {
				ContentAssemblyService caService = context
						.getContentAssemblyService();
				// Attach new MO to parent if not already there:
				ContentAssemblyNodeContainer caNode = caService
						.getContentAssemblyNodeContainer(sys, parentMoId);
				wfLog.info("Found specified parent CA Node "
						+ caNode.getDisplayName() + " [" + caNode.getId() + "]");
				...
				reportAndThrowRSuiteException("Exception attempting to attach new/updated MO to browse tree: "
						+ e.getMessage());
			}
		}
	...
	protected String getEffectiveAlias(WorkflowContext context,
			String defaultAlias) throws RSuiteException {
	...
	protected File getFileToLoad(WorkflowContext context)
		...
		if (fileToLoadFromWorkflow != null) {
			fileToLoad = resolveVariablesAndExpressions(
					fileToLoadFromWorkflow.getExpressionText(), "");
		}
		if (fileToLoad == null || "".equals(fileToLoad.trim())) {
			...
			if (!inFile.exists()) {
				throw new FileNotFoundException("Specified file to load \""
						+ fileToLoad + "\" not found.");
			}
		}
		return inFile;
	}
	...
	protected String getParentMoId(WorkflowContext context) {
		...
		String parentCaMoId = resolveVariablesAndExpressions(parentMoId
				.getExpressionText());

		// String parentMoId = context.getVariableAsString(PARENT_MO_ID_PARAM);
		if (StringUtils.isNotEmpty(parentCaMoId)) {
			return parentCaMoId;
		}
		parentCaMoId = resolveVariablesAndExpressions(getParameter(PARENT_MO_ID_PARAM));
		if (StringUtils.isNotEmpty(parentCaMoId)) {
			return parentCaMoId;
		}
		parentCaMoId = resolveExpression(parentMoIdFromWorkflow);
		context.getWorkflowLog().info(
				"Resolved expression [" + parentMoId + "]");
		return parentCaMoId;
	}
	...
	protected String getCommitMessage(WorkflowContext context) {
		String commitMessagewf = commitMessage.getExpressionText();
		if (StringUtils.isNotEmpty(commitMessagewf)) {
			return commitMessagewf;
		}
		commitMessagewf = context.getVariableAsString(COMMIT_MESSAGE_PARAM);
		if (StringUtils.isNotEmpty(commitMessagewf)) {
			return commitMessagewf;
		}
		commitMessagewf = resolveExpression(commitMessageFromWorkflow);
		context.getWorkflowLog().info(
				"Resolved expression [" + commitMessagewf + "]");
		return commitMessagewf;
	}
