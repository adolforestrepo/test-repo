...
public class Docx2XmlBean extends TransformSupportBean {
	...
	private String rootMapName = null;
	private String rootTopicName = null;

    /**
     * Construct new docx-to-xml converter. One of rootMapFilename or rootTopicFilename 
     * must be specified.
     * @param   context             RSuite workflow context.
     * @param   xsltUriString       DOX XSLT to use.
     * @param   styleMapUriString   Style mapping file.
     * @param   rootMapFilename     The filename of the root map file.
     * @param   rootTopicFilename   The filename of the root topic file.
     * @throws  RSuiteException     If an error occurs.
     */
	public Docx2XmlBean(
			WorkflowContext context, 
			String xsltUriString, 
			String styleMapUriString, 
			String rootMapFilename,
			String rootTopicFilename) throws RSuiteException {
		super(context, xsltUriString);
		
		this.styleMapUri = styleMapUriString;		
		if (rootMapFilename != null) {
			this.rootMapName = FilenameUtils.getBaseName(rootMapFilename);
		}
		if (rootTopicFilename != null) {
			this.rootTopicName = FilenameUtils.getBaseName(rootTopicFilename);
		}
	}
    ...
    ) throws RSuiteException
    {
		...
		if (this.rootMapName != null) {
			params.put("rootMapName", rootMapName);
		}
		if (this.rootTopicName != null) {
			params.put("rootTopicName", rootTopicName);
		}
		if (userParams != null)
