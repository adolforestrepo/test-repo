...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
  ...
  <xsl:template match="*[df:class(., 'topic/p')][@outputclass != '']" priority="7" 
    mode="style-map-pstyle">
    <xsl:message> + [DEBUG] p with non-empty outputclass TPM: <xsl:value-of select="$styleCatalog"/>  <xsl:value-of select="@outputclass"/></xsl:message>
    <xsl:sequence select="string(./@outputclass)"/>
  </xsl:template>
 ...
 <xsl:template match="*[df:class(., 'sidebar/sidebar')]/*[df:class(., 'topic/body')]//*[df:class(., 'topic/p') and @outputclass = 'CopySansNoIndent']" priority="15"
  ...
  <xsl:template match="*[df:class(., 'sidebar/sidebar')]/*[df:class(., 'topic/body')]//*[df:class(., 'topic/p') and @outputclass = 'CopySidebarNoIndent']" priority="15"
    mode="style-map-pstyle">
    <xsl:sequence select="'CopySidebarNoIndent'"/>
  </xsl:template>

  <xsl:template match="*[df:class(., 'sidebar/sidebar')]/*[df:class(., 'topic/body')]//*[df:class(., 'topic/p') and @outputclass = 'CopySidebar']" priority="15"
    mode="style-map-pstyle">
    <xsl:sequence select="'CopySidebar'"/>
  </xsl:template>
...
-->    
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:sequence select="'CopyIndent'"/>
      </xsl:otherwise>
    </xsl:choose>
  ...
  <xsl:template match="*[df:class(., 'astd-author-d/author-desc')]" 
    ...
    <xsl:sequence select="'authorDesc'"/>
  </xsl:template>
  ...
  <xsl:template match="*[df:class(., 'sidebar/sidebar')]/*[df:class(., 'topic/body')]//*[df:class(., 'topic/ul')]/*[df:class(., 'topic/li')]" priority="12"
    mode="style-map-pstyle">
    <xsl:sequence select="'CopySidebarBullet'"/>  
   <!-- <xsl:sequence select="'CopySansBullet'"/> -->
  </xsl:template>
  
    <xsl:template 
    match="*[df:class(., 'topic/ol')]/
    *[df:class(., 'topic/li')][count(preceding-sibling::*[df:class(.,'topic/li')]) = 0][@outputclass = 'CopySidebarNumberedList']"
    priority="15" 
    mode="style-map-pstyle">
    <xsl:sequence select="'CopySidebarNumberedList'"/>
<!--     <xsl:sequence select="'CopyNumbered'"/> -->
  ...
  <xsl:template 
    ...
    *[df:class(., 'topic/li')][count(preceding-sibling::*[df:class(.,'topic/li')]) = 0]"
    priority="10" 
    mode="style-map-pstyle">
    <xsl:sequence select="'CopyNumberedList'"/>
<!--     <xsl:sequence select="'CopyNumbered'"/> -->
  </xsl:template>
  ...
  <xsl:template match="*[df:class(., 'topic/ph')][@outputclass = 'boldface']" 
   ...
   <!-- maps the boldface style in incopy config styles
  <xsl:template match="*[df:class(., 'topic/ph')][@outputclass = 'italic']" 
    mode="style-map-cstyle">
