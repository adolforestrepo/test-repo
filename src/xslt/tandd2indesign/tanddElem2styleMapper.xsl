...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	xmlns:df="http://dita2indesign.org/dita/functions" xmlns:e2s="http//dita2indesign.org/functions/element-to-style-mapping"
	exclude-result-prefixes="xs local df e2s" version="2.0">

	<!-- Element-to-style mapper Maps DITA elements to T+D-specific InDesign 
		styles Copyright (c) 2009 ASTD -->

	<xsl:template
		match="/*[df:class(., 'topic/topic')]/*[df:class(., 'topic/title')][@outputclass = 'Headline']"
		mode="style-map-pstyle">
		<xsl:sequence select="'HeadlineDept'" />
	</xsl:template>

	<xsl:template
		match="/*[df:class(., 'topic/topic')]/*[df:class(., 'topic/title')][@outputclass = 'InFocusHeadDept']"
		mode="style-map-pstyle">
		<xsl:sequence select="'InFocusHeadDept'" />
	</xsl:template>

	<xsl:template
		match="/*[df:class(., 'topic/topic')]/*[df:class(., 'topic/title')][@outputclass = 'InFocusHead']"
		mode="style-map-pstyle">
	...
	<xsl:template
		match="/*[df:class(., 'topic/topic')]/*[df:class(., 'topic/topic')]/*[df:class(., 'topic/title')]"
		mode="style-map-pstyle">
		<xsl:sequence select="'Subhead'" />
	</xsl:template>

	<xsl:template
		match="/*[df:class(., 'topic/topic')]/*[df:class(., 'topic/topic')]/*[df:class(., 'topic/topic')]/*[df:class(., 'topic/title')]"
		mode="style-map-pstyle">
		<xsl:sequence select="'SidebarSubhead'" />
	</xsl:template>

	<xsl:template
		match="*[df:class(., 'sidebar/sidebar')]/*[df:class(., 'topic/title')]"
		mode="style-map-pstyle" priority="10">
		<xsl:sequence select="'SidebarHead'" />
	</xsl:template>

	<xsl:template match="*[df:class(., 'topic/p')][@outputclass != '']"
		priority="7" mode="style-map-pstyle">
		<xsl:message>
			+ [DEBUG] p with non-empty outputclass:
			<xsl:value-of select="@outputclass" />
		</xsl:message>
		<xsl:sequence select="string(./@outputclass)" />
	</xsl:template>

	<xsl:template match="*[df:class(., 'topic/p')][@outputclass = 'BookTitle']"
		mode="style-map-pstyle">
		<xsl:message>
			+ [DEBUG] p with outputclass='BookTitle':
			<xsl:value-of select="@outputclass" />
		</xsl:message>
		<xsl:sequence select="string(./@outputclass)" />
	</xsl:template>

	<xsl:template match="*[df:class(., 'topic/p')][@outputclass = 'BookSpecs']"
		mode="style-map-pstyle">
		<xsl:message>
			+ [DEBUG] p with outputclass='BookSpecs':
			<xsl:value-of select="@outputclass" />
		</xsl:message>
	...
	<xsl:template
		match="*[df:class(., 'topic/p')][@outputclass = 'InFocusHeadDept']"
		mode="style-map-pstyle">
		<xsl:message>
			+ [DEBUG] p with outputclass='InFocusHeadDept':
			<xsl:value-of select="@outputclass" />
		</xsl:message>
		<xsl:sequence select="'InFocusHeadDept'" />
	</xsl:template>

	<xsl:template match="*[df:class(., 'topic/p')][@outputclass = 'InFocusHead']"
		mode="style-map-pstyle">
		<xsl:message>
			+ [DEBUG] p with outputclass='InFocusHead':
			<xsl:value-of select="@outputclass" />
		</xsl:message>
		<xsl:sequence select="'InFocusHead'" />
	</xsl:template>
	
	<xsl:template match="*[df:class(., 'topic/p')][@outputclass = 'BookQuote']"
		mode="style-map-pstyle">
		<xsl:message>
			+ [DEBUG] p with outputclass='BookQuote':
			<xsl:value-of select="@outputclass" />
		</xsl:message>
	...
	<xsl:template match="*[df:class(., 'topic/p')][@outputclass = 'BigNumber']"
		mode="style-map-pstyle">
		<xsl:message>
			+ [DEBUG] p with outputclass='BigNumber':
			<xsl:value-of select="@outputclass" />
		</xsl:message>
	...
	<xsl:template match="*[df:class(., 'topic/p')][@outputclass = 'Tag']"
		mode="style-map-pstyle">
		<xsl:message>
			+ [DEBUG] p with outputclass='Tag':
			<xsl:value-of select="@outputclass" />
		</xsl:message>
	...
	<xsl:template
		match="*[df:class(., 'topic/p')][@outputclass = 'SidebarHeadDept']"
		mode="style-map-pstyle">
		<xsl:message>
			+ [DEBUG] p with outputclass='SidebarHeadDept':
			<xsl:value-of select="@outputclass" />
		</xsl:message>
		<xsl:sequence select="'SidebarHeadDept'" />
	</xsl:template>

	<xsl:template match="*[df:class(., 'topic/p')][@outputclass = 'SmallSans']"
		mode="style-map-pstyle">
		<xsl:message>
			+ [DEBUG] p with outputclass='SmallSans':
			<xsl:value-of select="@outputclass" />
		</xsl:message>
	...
	<xsl:template
		match="*[df:class(., 'topic/p')][@outputclass = 'CopySansNumbered']"
		mode="style-map-pstyle">
		<xsl:message>
			+ [DEBUG] p with outputclass='CopySansNumbered':
			<xsl:value-of select="@outputclass" />
		</xsl:message>
	...
	<xsl:template
		match="*[df:class(., 'topic/p')][@outputclass = 'CopySansUpsized']"
		mode="style-map-pstyle">
		<xsl:message>
			+ [DEBUG] p with outputclass='CopySansUpsized':
			<xsl:value-of select="@outputclass" />
		</xsl:message>
	...
	<xsl:template
		match="*[df:class(., 'sidebar/sidebar')]/*[df:class(., 'topic/body')]//*[df:class(., 'topic/p')]"
		mode="style-map-pstyle">
	...
	<xsl:template match="*[df:class(., 'topic/p')]" priority="5"
		...
		<xsl:param name="articleType" as="xs:string" tunnel="yes"
			select="'article'" />
		<!-- <xsl:message> + [DEBUG] (style-map-pstyle) topic/p: Article type is 
			"<xsl:sequence select="$articleType"/>"</xsl:message> -->
		<xsl:choose>
	...
	<xsl:template match="*[df:class(., 'astd_p-d/copy-block')]"
		priority="10" mode="style-map-pstyle">
		<xsl:sequence select="'CopyBlock'" />
	</xsl:template>

	<xsl:template match="*[df:class(., 'topic/p') and @outputclass != '']"
		priority="5" mode="style-map-pstyle">
		<!-- By default, if there is an output classs on 'p', assume it's the InDesign 
			style. -->
		<xsl:sequence select="string(@outputclass)" />
	</xsl:template>
	...
	<xsl:template match="*[df:class(., 'topic/lq')]/*[df:class(., 'topic/p')]"
		priority="10" mode="style-map-pstyle">
	...
	<xsl:template match="*[df:class(., 'article/deck')]" mode="style-map-pstyle"
		priority="10">
		<xsl:sequence select="'DeckDept'" />
	</xsl:template>

	<!-- Depreciated since #93 <xsl:template match="/*[df:class(., 'article/article')]/*[df:class(., 
		'topic/body')]/*[df:class(., 'topic/p')][count(preceding-sibling::*[df:class(.,'topic/p')]) 
		= 0]" mode="style-map-pstyle" priority="15"> <xsl:sequence select="'Copy 
		Intro'"/> </xsl:template> -->
	<xsl:template match="*[df:class(., 'astd-bodydiv/callout')]"
		mode="style-map-pstyle">
	...
	<!-- added forticket #93 -->
	<xsl:template
		match="*[df:class(., 'sidebar/sidebar')]/*[df:class(., 'topic/body')]//*[df:class(., 'topic/ul')]/*[df:class(., 'topic/li')]"
    ...
    *[df:class(., 'topic/li')][count(preceding-sibling::*[df:class(.,'topic/li')]) = 0]"
	...
	<xsl:template match="*[df:class(., 'topic/dt')]" mode="style-map-cstyle">
	...
	<xsl:template
		match="*[df:class(., 'topic/ol')]/*[df:class(., 'topic/li')][@outputclass = 'CopyNumberedList']"
		mode="style-map-pstyle">
		<xsl:sequence select="'CopyNumbered'" />
	</xsl:template>
	...
	<xsl:template
		match="*[df:class(., 'topic/fig')]/*[df:class(., 'topic/title')]"
		mode="style-map-pstyle">
	...
	<xsl:template match="*[df:class(., 'hi-d/i')]" mode="style-map-cstyle"
		priority="10">
		<xsl:sequence select="'CopyItalic'" />
	</xsl:template>
	...
	<xsl:template
		match="/*[df:class(., 'topic/topic')]/*[df:class(., 'interview-question/interview-question')]/*[df:class(., 'topic/title')]"
		mode="style-map-pstyle" priority="10">
