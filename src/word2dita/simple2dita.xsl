...
<!--    <xsl:variable name="doDebug" as="xs:boolean" select="true()"/>-->
  ...
  <xsl:template match="rsiwp:table">
    ...
    <xsl:element name="{$tagName}">  
      ...
      <xsl:attribute name="outputclass" select="@table_style_name"/>
      <tgroup>
...
<?remove  
  ...
  <xsl:function name="local:isTopicTitle" as="xs:boolean">
    <xsl:param name="context" as="element()"/>
    <!-- RSI replaced the definition of the styleId variable to avoid error when using RSuite's version of Saxon-->
    <xsl:variable name="styleId" as="xs:string">
      <xsl:choose>
        <xsl:when test="$context/@styleName != ''">
          <xsl:sequence select="$context/@styleName"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:sequence select="$context/@styleId"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    <xsl:choose>
      ...
      <xsl:otherwise>
        <!-- RSI replaced the definition of the styleMap variable to avoid error when using RSuite's version of Saxon-->
        <xsl:variable name="styleMap" as="element()?">
          <xsl:choose>
            <xsl:when test="key('styleMapsByName', lower-case($styleId), $styleMapDoc)[1]/text() != ''">
              <xsl:sequence select="(key('styleMapsByName', lower-case($styleId), $styleMapDoc))[1]"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:sequence select="(key('styleMapsById', $styleId, $styleMapDoc))[1]"/>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:variable>        
        <xsl:sequence
